...
(describe button
          ...
          (it "should handle the :margin option with to-insets"
              (let [b (button :margin 1)
                    i   (.getMargin b)]
                (expect (= [1 1 1 1] [(.top i) (.left i) (.bottom i) (.right i)])))) (it "should add the button to a button group specified with the :group option"
                                                                                         (let [bg (button-group)
                                                                                               b  (button :group bg)]
                  ...
                  (let [dlg (custom-dialog :content "Nothing" :modal? true)]
                    (expect (= (test-dlg-blocking
                                dlg :future-fn #(do
                                                  ...
                                                  (Thread/sleep 50))) :ok)))))) (describe dialog
                                                                                          (it "should block until dialog is being disposed of"
